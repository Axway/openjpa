...
public class TestLRUCache extends SingleEMFTestCase {
    ...
    public void testQueryCacheOverFlow() {
        ...
        for (int i = 0; i < cacheSize + 1; i++) {
            ...
            person.setId(i + 1); // FIXME Axway - ApplicationIds.create: avoid using 0 as an ID work around bug introduced in ApplicationIds.create
            em.persist(person);
        }
        ...
        for (int i = 0; i < cacheSize + 1; i++) {
            em.createQuery(QUERY + (i + 1), CachedPerson.class).getSingleResult(); // FIXME Axway - ApplicationIds.create: avoid using 0 as an ID work around bug introduced in ApplicationIds.create
        }
        ...
        for (int i = 0; i < keys.size(); i++) {
            boolean res = contains(QUERY + (i + 1), strKeys); // FIXME Axway - ApplicationIds.create: avoid using 0 as an ID work around bug introduced in ApplicationIds.create
            if (i == 0) {
                assertFalse(res);
            } else {
                assertTrue(res);
    ...
    public void testDataCacheOverFlow() {
        ...
        for (int i = 0; i < cacheSize + 1; i++) {
            IntId id = new IntId(CachedPerson.class, i + 1 ); // FIXME Axway - ApplicationIds.create: avoid using 0 as an ID work around bug introduced in ApplicationIds.create
