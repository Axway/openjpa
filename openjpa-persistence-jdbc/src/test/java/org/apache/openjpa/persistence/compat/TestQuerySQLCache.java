    ...
    private void runMultiEMCaching(Map props) {
        ...
        try {
            ...
            for (int i = 0; i < nPeople; i++) {
                Person p = new Person();
                p.setId(i + 1); // Axway - Avoid using 0 as an id to work around bug introduced in ApplicationIds.create()
                em.persist(p);
            }
            em.flush();
            em.getTransaction().commit();
            ...
            for (int i=0; i < nThreads; i++) {
                ...
                // Axway - workaround bug introduced in ApplicationIds.create() by adding 1
